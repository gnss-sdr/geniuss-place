...
```
1. Checkout the newly fetched `main` branch and change the `HEAD` pointer to it:
```console
$ git checkout main
$ git symbolic-ref refs/remotes/origin/HEAD refs/remotes/origin/main
```
If you only cloned the repository from upstream, then you are good to go.
1. If you forked from upstream and then cloned your own repo, then there is some
more extra work to do. Push the local `main` branch to your own repo:
```console
git push -u origin main
```
1. Change the default branch name to `main` on your forked GitHub repository.
