...
The tags are obtained by generating a certain MAC, following the specific information within the Tag-Info field, and then truncating it (starting from the MSB) to the length defined by the Tag Size field within the DSM-KROOT of the chain in force.
  ...
  ![MACK Tag-Info]({{ "/assets/images/MACK_tag_info.png" | relative_url }}){:width="250px"}{: .align-center .invert-colors}
  *MACK message's Tag-Info field (from OSNMA SIS-ICD [^OSNMA-ICD]), where the $$ \text{PRN}_{D} $$ field identifies the satellite transmitting the navigation data which is authenticated by the tag.*
  {: style="text-align: center;"}
