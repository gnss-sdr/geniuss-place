...
| `adc_bits` | [`2`, `4`]: Selects 2 or 4 bit I/Q quantization. **Options for 8 and 16 bits are not implemented.** It defaults to 4 bits. | Optional |
| `endian` | [`true`, `false`]: If it is set to `false`, it assumes that the host machine is reading in Big Endian (that is, the the byte containing the most significant bit is stored first and has the lowest memory address). If it is set to `true`, it assumes Little Endian reading (that is, the the byte containing the most significant bit is stored last and has the highest address). It defaults to `false`. | Optional |
| `samples` | Number of samples to be read. If set to $$ 0 $$ the whole file but the last two milliseconds are processed. It defaults to $$ 0 $$. | Optional |
...
LabSat 3 splits data in 2.1 GB files. This file source automatically increments the file name when the signal is splitted in several files: it adds "_0000.LS3" to this base path and filename. Next file will be "_0001.LS3" and so on.
