...
| `enable_fll_steady_state` | [`true`, `false`]. If set to `true`, the FLL is enabled beyond the pull-in stage. It defaults to `false`. <span style="color: orange">This parameter is only present in the `next` branch, and will be included in the next stable release.</span> | Optional |
...
| `cn0_smoother_samples` | Number of samples used to smooth the value of the estimated $$ C/N_0 $$. It defaults to 200 samples. <span style="color: orange">This parameter is only present in the `next` branch, and will be included in the next stable release.</span> | Optional |
| `cn0_smoother_alpha` | Forgetting factor of the $$ C/N_0 $$ smoother, as in $$ y_k = \alpha x_k + (1 - \alpha) y_{k-1} $$. It defaults to 0.002. <span style="color: orange">This parameter is only present in the `next` branch, and will be included in the next stable release.</span> | Optional |
| `carrier_lock_test_smoother_samples` | Number of samples used to smooth the value of the carrier lock test. It defaults to 25 samples. <span style="color: orange">This parameter is only present in the `next` branch, and will be included in the next stable release.</span> | Optional |
| `carrier_lock_test_smoother_alpha` | Forgetting factor of the carrier lock detector smoother, as in $$ y_k = \alpha x_k + (1 - \alpha) y_{k-1} $$. It defaults to 0.002. <span style="color: orange">This parameter is only present in the `next` branch, and will be included in the next stable release.</span> | Optional |
