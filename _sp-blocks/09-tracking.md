...
Optionally, a passive
[frequency-error reduction scan](#frequency-error-reduction-scan), not shown in
the figure, can be inserted between the pull-in state and the closed-loop
tracking states in order to re-measure the Doppler shift before the loops are
closed.
...
## Frequency-error reduction scan

The `Tracking_<signal>.f_error_step_num` option enables a Doppler refinement
scan that the DLL/PLL tracking implementations run every time a channel starts
tracking a satellite, right after the pull-in state and before closing the
tracking loops. It defaults to `0` (scan disabled).
...
A navigation-bit or secondary-code sign transition falling inside the coherent
integration window of the _Acquisition_ block splits the correlation peak and
can displace the Doppler estimate $$ \hat{f}_{\!D_{acq}} $$ handed over to
tracking by more than one acquisition bin. The frequency discriminator described
in the [Discriminators](#discriminators) section below has an unambiguous range
of $$ \pm \frac{1}{2 T_{int}} $$, so for signals with long primary code
periods, such as Galileo E1 ($$ T_{int} = 4 $$ ms, that is, $$ \pm 125 $$ Hz),
such a displacement can exceed what the FLL is able to pull in, and the loops
may take long to converge or fail to lock. The scan re-measures the Doppler
shift with the tracking correlators before the loops are closed:

* Once the pull-in state has aligned the local code replica with the code phase
  estimated by the _Acquisition_ block, the channel tests `f_error_step_num`
  Doppler hypotheses centered on $$ \hat{f}_{\!D_{acq}} $$ and spaced
  `f_error_doppler_step` Hz apart, visited in the order $$ 0, +1, -1, +2, -2,
  \ldots $$ times the step. Even values of `f_error_step_num` are rounded up to
  the next odd number, so that the center bin is always accompanied by the same
  number of positive and negative offsets.
* For each hypothesis, the local carrier is set to that Doppler shift and the
  power at the output of the Prompt correlator, $$ \vert P[m] \vert^2 $$, is
  non-coherently accumulated over `f_error_accumulation` code periods. The scan
  is passive: the code phase and the state of the loop filters are left exactly
  as the pull-in state set them, and no discriminator runs while it lasts.
* When all bins have been tested, the Doppler shift of the bin with the highest
  accumulated power is retained, the carrier loop filter is re-initialized with
  it, and the channel proceeds to the usual closed-loop tracking states. The
  selected offset is reported in the log at `INFO` level.

The scan delays the start of closed-loop tracking, and hence bit or
secondary-code synchronization and the first position fix, by
`f_error_step_num` $$ \times $$ `f_error_accumulation` code periods per channel:
for instance, 9 bins with 5 accumulations each on Galileo E1 (4 ms code period)
take 180 ms. The `pull_in_time_s` and `bit_synchronization_time_limit_s` budgets
start counting when the scan completes, so the loops keep their full settling
time. The deliberate frequency steps between bins are not interpreted as signal
dynamics by the Doppler-rate estimator enabled with the `high_dyn` option.

Since each bin is measured with a coherent integration of one code period, the
accumulated power follows a $$ \text{sinc}^2 $$ envelope with first nulls at
$$ \pm 1 / T_{int} $$ around the true Doppler shift, and the scan can only
discriminate steps that are a fraction of that width. For signals with short
code periods and sparse bit transitions, such as GPS L1 C/A (1 ms codes, 20 ms
bits), a displaced estimate is infrequent, the FLL pull-in range is wider, and
the resolution of a one-code-period scan is coarse, so the benefit is small.
The option is intended mainly for Galileo E1 and other signals with long code
periods. Choosing `f_error_doppler_step` well below $$ 1 / T_{int} $$, and a
number of bins such that the total span $$ \pm \frac{N-1}{2} \Delta f $$ (where
$$ N $$ is `f_error_step_num` and $$ \Delta f $$ is `f_error_doppler_step`)
covers at least one acquisition Doppler bin on each side, allows the scan to
correct the quantization error of the acquisition grid as well as a
bit-transition displacement. Longer accumulations improve the estimate at low
$$ C/N_0 $$ at the cost of a longer scan.

Each completed scan is also written to the CSV file set by
`f_error_dump_filename`, which defaults to `./f_error_dump.csv`; set this
parameter to an empty value to disable the output. Channels sharing a file name
write to the same file: the first scan of a receiver run overwrites any previous
file and writes a header line, and later scans append their rows. Each row
contains `scan_id` (a counter identifying the scan), `prn`, `system_char` (the
ASCII code of the system identifier, _e.g._, 71 for `G` and 69 for `E`),
`channel`, `cn0_dBHz` (an estimate of $$ C/N_0 $$ obtained from the selected
bin), `selected_doppler_hz`, `selected_bin` (position of the selected bin in
increasing Doppler order), `bin_index`, `doppler_hz`, and `power`, with one row
per tested bin, sorted by increasing Doppler. The file can be read directly in
Matlab/Octave, _e.g._, with `scans = csvread('f_error_dump.csv', 1, 0);`.

For example, this Galileo E1 configuration, in which the acquisition grid has a
Doppler step of 125 Hz, tests nine bins spaced 31.25 Hz apart (a span of
$$ \pm 125 $$ Hz, that is, one full acquisition bin on each side), accumulating
five 4 ms code periods per bin, for a total scan time of 180 ms:
...
Tracking_1B.f_error_step_num=9
Tracking_1B.f_error_doppler_step=31.25
Tracking_1B.f_error_accumulation=5
Tracking_1B.f_error_dump_filename=
```

This option is supported by the implementations whose parameter tables below
include `f_error_step_num`.
...
|          `f_error_step_num`          | Number of Doppler bins tested by the frequency-error reduction scan run after the pull-in stage, centered on the Doppler estimate provided by the _Acquisition_ block. Even values are rounded up to the next odd number. It defaults to 0 (scan disabled). See [Frequency-error reduction scan](#frequency-error-reduction-scan) for availability, tradeoffs, and interactions.                                                                                             |   Optional   |
|        `f_error_doppler_step`        | Spacing between consecutive Doppler bins of the frequency-error reduction scan, in Hz. Only used if `f_error_step_num` $$ > $$ 0. It defaults to 250 Hz.                                                                                                                                                                                                                                                                                                                     |   Optional   |
|        `f_error_accumulation`        | Number of code periods over which the Prompt correlator power is non-coherently accumulated in each Doppler bin of the frequency-error reduction scan. Only used if `f_error_step_num` $$ > $$ 0; a value of 0 is replaced by 1. It defaults to 20.                                                                                                                                                                                                                          |   Optional   |
|       `f_error_dump_filename`        | Name of the CSV file in which the results of each frequency-error reduction scan are written. Only used if `f_error_step_num` $$ > $$ 0. Set it to an empty value to disable this output. It defaults to `./f_error_dump.csv`.                                                                                                                                                                                                                                               |   Optional   |
...
## BeiDou B1I signal tracking

### Implementation: `BEIDOU_B1I_DLL_PLL_Tracking`
...
| :----------------------------------: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------: |
|            --------------            |
|           `implementation`           | `BEIDOU_B1I_DLL_PLL_Tracking`                                                                                                                                                                                                                                                                                                                                                                                                                                                 |  Mandatory   |
|             `item_type`              | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>.                                                                             |   Optional   |
|     `extend_correlation_symbols`     | Sets the number of correlation symbols to be extended after bit synchronization has been achieved. Each symbol is 1 ms, so setting this parameter to 20 means a coherent integration time of 20 ms. Each bit is 20 ms, so the value of this parameter must be a divisor of it (_e.g._, 2, 4, 5, 10, 20). The higher this parameter is, the better local clock stability will be required. It defaults to 1.                                                                  |   Optional   |
|             `pll_bw_hz`              | Bandwidth of the PLL low-pass filter, in Hz. It defaults to 50 Hz.                                                                                                                                                                                                                                                                                                                                                                                                           |   Optional   |
|          `pll_bw_narrow_hz`          | Bandwidth of the PLL low-pass filter after bit synchronization, in Hz. It defaults to 20 Hz.                                                                                                                                                                                                                                                                                                                                                                                 |   Optional   |
...
|            --------------            |
  ...
  _Tracking implementation:_ **`BEIDOU_B1I_DLL_PLL_Tracking`**.
  {: style="text-align: center;"}
...
```ini
;######### TRACKING CONFIG FOR BeiDou B1I CHANNELS ############
Tracking_B1.implementation=BEIDOU_B1I_DLL_PLL_Tracking
Tracking_B1.item_type=gr_complex
Tracking_B1.extend_correlation_symbols=20
Tracking_B1.early_late_space_chips=0.5;
Tracking_B1.early_late_space_narrow_chips=0.1;
Tracking_B1.pll_bw_hz=35;
Tracking_B1.dll_bw_hz=2.0;
Tracking_B1.pll_bw_narrow_hz=5.0;
Tracking_B1.dll_bw_narrow_hz=0.50;
Tracking_B1.fll_bw_hz=10
...
| :----------------------------------: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------: |
|            --------------            |
|           `implementation`           | `QZSS_L1_CA_DLL_PLL_Tracking`                                                                                                                                                                                                                                                                                                                                                                                                                                                 |  Mandatory   |
|             `item_type`              | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>.                                                                             |   Optional   |
|     `extend_correlation_symbols`     | Sets the number of correlation symbols to be extended after bit synchronization has been achieved. Each symbol is 1 ms, so setting this parameter to 20 means a coherent integration time of 20 ms. Each bit is 20 ms, so the value of this parameter must be a divisor of it (_e.g._, 2, 4, 5, 10, 20). The higher this parameter is, the better local clock stability will be required. It defaults to 1.                                                                  |   Optional   |
|             `pll_bw_hz`              | Bandwidth of the PLL low-pass filter, in Hz. It defaults to 50 Hz.                                                                                                                                                                                                                                                                                                                                                                                                           |   Optional   |
|          `pll_bw_narrow_hz`          | Bandwidth of the PLL low-pass filter after bit synchronization, in Hz. It defaults to 20 Hz.                                                                                                                                                                                                                                                                                                                                                                                 |   Optional   |
...
|            --------------            |
  ...
  _Tracking implementation:_ **`QZSS_L1_CA_DLL_PLL_Tracking`**.
  {: style="text-align: center;"}
...
```ini
;######### TRACKING CONFIG FOR QZSS L1 C/A CHANNELS ############
Tracking_J1.implementation=QZSS_L1_CA_DLL_PLL_Tracking
Tracking_J1.item_type=gr_complex
Tracking_J1.extend_correlation_symbols=20
Tracking_J1.early_late_space_chips=0.5;
Tracking_J1.early_late_space_narrow_chips=0.1;
Tracking_J1.pll_bw_hz=35;
Tracking_J1.dll_bw_hz=2.0;
Tracking_J1.pll_bw_narrow_hz=5.0;
Tracking_J1.dll_bw_narrow_hz=0.50;
Tracking_J1.fll_bw_hz=10
Tracking_J1.enable_fll_pull_in=true;
Tracking_J1.enable_fll_steady_state=false
Tracking_J1.dump=false
Tracking_J1.dump_filename=tracking_ch_
```


## SBAS L1 signal tracking

The SBAS L1 signal (GNSS-SDR identifier `S1`), broadcast at $$ 1575.42 $$ MHz
by the geostationary satellites of the different satellite-based augmentation
systems (EGNOS, WAAS, ...), uses BPSK-modulated Gold codes of $$ 1023 $$ chips
at $$ 1.023 $$ Mchip/s (the same code family as GPS L1 C/A) and has no pilot
component. All the signal power is in the data channel, which carries
navigation data at $$ 250 $$ bits per second, convolutionally encoded at rate
$$ 1/2 $$ into $$ 500 $$ symbols per second, so the encoded symbol period is
$$ 2 $$ ms ($$ 2 $$ code periods). Since the transmitting satellites are
geostationary, the signal dynamics observed by a static receiver are very low,
and narrow DLL / PLL bandwidths can be used.

### Implementation: `SBAS_L1_DLL_PLL_Tracking`
...
| :----------------------------------: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------: |
|            --------------            |
|           `implementation`           | `SBAS_L1_DLL_PLL_Tracking`                                                                                                                                                                                                                                                                                                                                                                                                                                                    |  Mandatory   |
|             `item_type`              | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>.                                                                             |   Optional   |
|     `extend_correlation_symbols`     | For SBAS L1, coherent integration is limited to $$ 1 $$ code period ($$ 1 $$ ms), since the SBAS Telemetry Decoder performs its own, unsynchronized sample-to-symbol pairing, and an extended integration would straddle an encoded symbol boundary. This parameter defaults to 1, and values greater than 1 are clamped back to 1 with a warning.                                                                                                                              |   Optional   |
|             `pll_bw_hz`              | Bandwidth of the PLL low-pass filter, in Hz. It defaults to 50 Hz.                                                                                                                                                                                                                                                                                                                                                                                                           |   Optional   |
|          `pll_filter_order`          | [`2`, `3`]. Sets the order of the PLL low-pass filter. It defaults to 3.                                                                                                                                                                                                                                                                                                                                                                                                     |   Optional   |
|         `enable_fll_pull_in`         | [`true`, `false`]. If set to `true`, enables the FLL during the pull-in time. It defaults to `false`.                                                                                                                                                                                                                                                                                                                                                                        |   Optional   |
|      `enable_fll_steady_state`       | [`true`, `false`]. If set to `true`, the FLL is enabled beyond the pull-in stage. It defaults to `false`.                                                                                                                                                                                                                                                                                                                                                                    |   Optional   |
...
|            --------------            |
  ...
  _Tracking implementation:_ **`SBAS_L1_DLL_PLL_Tracking`**.
  {: style="text-align: center;"}
...
```ini
;######### TRACKING CONFIG FOR SBAS L1 CHANNELS ############
Tracking_S1.implementation=SBAS_L1_DLL_PLL_Tracking
Tracking_S1.item_type=gr_complex
Tracking_S1.extend_correlation_symbols=1
Tracking_S1.pll_bw_hz=25.0
Tracking_S1.dll_bw_hz=1.0
Tracking_S1.dump=false
Tracking_S1.dump_filename=tracking_ch_
```
...
| :----------------------------------: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------: |
|            --------------            |
|           `implementation`           | `QZSS_L5_DLL_PLL_Tracking`                                                                                                                                                                                                                                                                                                                                                                                                                                                    |  Mandatory   |
|             `item_type`              | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>.                                                                             |   Optional   |
|            `track_pilot`             | [`true`, `false`]: If set to `true`, the receiver is set to track the pilot signal L5Q and enables an extra prompt correlator (slave to pilot's prompt) in the data component L5I. If set to `false`, the receiver performs correlations on a data length of 1 ms over the L5I component. This parameter defaults to `true`.                                                                                                                                                 |   Optional   |
|     `extend_correlation_symbols`     | If `track_pilot=true`, sets the number of correlation symbols to be extended after the secondary code $$ C_{nh_{20}} $$ is removed from the pilot signal, in number of symbols. Each symbol is 1 ms, so setting this parameter to 25 means a coherent integration time of 25 ms. The higher this parameter is, the better local clock stability will be required. It defaults to 1.                                                                                          |   Optional   |
|             `pll_bw_hz`              | Bandwidth of the PLL low-pass filter, in Hz. It defaults to 50 Hz.                                                                                                                                                                                                                                                                                                                                                                                                           |   Optional   |
...
|            --------------            |
  ...
  _Tracking implementation:_ **`QZSS_L5_DLL_PLL_Tracking`**.
  {: style="text-align: center;"}

Example:

```ini
;######### TRACKING CONFIG FOR QZSS L5 CHANNELS ############
Tracking_J5.implementation=QZSS_L5_DLL_PLL_Tracking
Tracking_J5.item_type=gr_complex
Tracking_J5.track_pilot=true
Tracking_J5.pll_bw_hz=20.0;
Tracking_J5.dll_bw_hz=1.5;
Tracking_J5.extend_correlation_symbols=20
Tracking_J5.pll_bw_narrow_hz=6.0
Tracking_J5.dll_bw_narrow_hz=0.5
Tracking_J5.enable_fll_pull_in=true
Tracking_J5.enable_fll_steady_state=false
Tracking_J5.fll_bw_hz=4.0
Tracking_J5.early_late_space_chips=0.5;
...
### Implementation: `BEIDOU_B3I_DLL_PLL_Tracking`
...
| :----------------------------------: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------: |
|            --------------            |
|           `implementation`           | `BEIDOU_B3I_DLL_PLL_Tracking`                                                                                                                                                                                                                                                                                                                                                                                                                                                    |  Mandatory   |
|             `item_type`              | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>.                                                                             |   Optional   |
|            `track_pilot`             | [`true`, `false`]: If set to `true`, the receiver is set to track the pilot signal L5Q and enables an extra prompt correlator (slave to pilot's prompt) in the data component L5I. If set to `false`, the receiver performs correlations on a data length of 1 ms over the L5I component. This parameter defaults to `true`.                                                                                                                                                 |   Optional   |
|     `extend_correlation_symbols`     | If `track_pilot=true`, sets the number of correlation symbols to be extended after the secondary code $$ C_{nh_{20}} $$ is removed from the pilot signal, in number of symbols. Each symbol is 1 ms, so setting this parameter to 25 means a coherent integration time of 25 ms. The higher this parameter is, the better local clock stability will be required. It defaults to 1.                                                                                          |   Optional   |
|             `pll_bw_hz`              | Bandwidth of the PLL low-pass filter, in Hz. It defaults to 50 Hz.                                                                                                                                                                                                                                                                                                                                                                                                           |   Optional   |
...
|            --------------            |
  ...
  _Tracking implementation:_ **`BEIDOU_B3I_DLL_PLL_Tracking`**.
  {: style="text-align: center;"}

Example:

```ini
;######### TRACKING CONFIG FOR BeiDOu B3I CHANNELS ############
Tracking_B3.implementation=BEIDOU_B3I_DLL_PLL_Tracking
Tracking_B3.item_type=gr_complex
Tracking_B3.track_pilot=true
Tracking_B3.pll_bw_hz=20.0;
Tracking_B3.dll_bw_hz=1.5;
Tracking_B3.extend_correlation_symbols=20
Tracking_B3.pll_bw_narrow_hz=6.0
Tracking_B3.dll_bw_narrow_hz=0.5
Tracking_B3.enable_fll_pull_in=true
Tracking_B3.enable_fll_steady_state=false
Tracking_B3.fll_bw_hz=4.0
Tracking_B3.early_late_space_chips=0.5;
Tracking_B3.early_late_space_narrow_chips=0.1;
