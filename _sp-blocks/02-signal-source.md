...
The GSS6450 data format is a Spirent/Keysight `.gns` file format used by
several GNSS RF record-and-playback products. It stores digitized GNSS RF
signals, and optionally in-band interference, so that captures can be replayed
or processed offline.

This block reads GSS6450-compatible `.gns` files and delivers samples in format
<abbr id="data-type" title="Complex samples with real and imaginary parts of type
32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>.

GSS6450 `.gns` files contain 32-bit little-endian words. Each word packs one or
more complex samples from a single recorded channel, with the in-phase component
stored before the quadrature component. The supported GSS6450 quantizations are
2, 4, 8, and 16 bits per I and Q component.

A GSS6450 recording normally stores one file per FPGA. Files from FPGA A use the
suffix `.A.gns`, files from FPGA B use `.B.gns`, and each file normally contains
one or two interleaved recorded channels. GSS6425 compatibility mode stores up
to three channels in one `.gns` file. A four-channel GSS6450 recording is
therefore represented as two signal sources, one for the `.A.gns` file and one
for the `.B.gns` file.

The source reads the embedded GSS6450/GSS6425 `.gns` header and startup
synchronization pattern when possible. If `adc_bits`, `total_channels`, or
`bytes_to_skip` are omitted, the block attempts to auto-detect the ADC
resolution, number of interleaved channels in the selected file, GSS6425
compatibility mode, and the first IQ data word after the header and
synchronization pattern. The `.gns` header does not generally convey the sample
rate, so set `sampling_frequency` explicitly in the GNSS-SDR configuration using
one of the valid recording rates below.

The `sampling_frequency` value is the sample rate of each recorded channel, in
samples per second. GSS6450 recordings use the following discrete rates:

| Mode | `sampling_frequency` value |
| :--- | -------------------------: |
| GSS6425 compatibility x1 | `10230000` |
| GSS6450 native x1 | `10305778` |
| x3 | `30690000` |
| x5 | `51150000` |
| x6 | `61380000` |
| x8 | `81840000` |
...
The GSS6450 native x1 rate is exactly `10.23 * 136 / 135 MHz`, so the integer
configuration value is rounded to `10305778` samples per second.
...
| :-----------------------: | :-------------- | :----------: |
|      --------------       |
|     `implementation`      | `GSS6450_File_Signal_Source` (`Spir_GSS6450_File_Signal_Source` is accepted as a legacy alias) |  Mandatory   |
|        `filename`         | Path to the `.gns` file containing the raw digitized I&Q samples. |  Mandatory   |
|   `sampling_frequency`    | Sample rate, in samples per second, for each recorded channel in the file. Use one of the GSS6450/GSS6425 rates listed above. |  Mandatory   |
|     `total_channels`      | Number of interleaved recorded channels present in this file. If omitted, the block attempts to infer it from the synchronization pattern. Valid values are `1` to `4`; GSS6450 files normally use `1` or `2`, and GSS6425 compatibility files can use `3`. |   Optional   |
|      `RF_channels`        | Number of RF channels exposed by this signal source to the rest of the receiver. If omitted, all decoded file channels are exposed. To expose only one channel from a multi-channel file, set `total_channels` to the file channel count, set `RF_channels=1`, and select the desired channel with `sel_ch`. |   Optional   |
|         `sel_ch`          | Selected file channel when exposing a single RF channel. 1 $$ <= $$ `sel_ch` $$ <= $$ `total_channels`. It defaults to `1`. |   Optional   |
|        `adc_bits`         | [`2`, `4`, `8`, `16`]: ADC resolution per I and Q component. If omitted, the block attempts to infer it from the embedded GSS6450 header. |   Optional   |
| `gss6425_compatible`      | [`true`, `false`]: If set to `true`, 2-bit samples are decoded using the GSS6425 compatibility mapping. If omitted, this is enabled automatically for files whose embedded header is `<Header> GSS6425`. |   Optional   |
|         `endian`          | [`true`, `false`]: If set to `true`, the block swaps the byte order of each 32-bit word before unpacking. GSS6450 files are stored as little-endian 32-bit words, so this usually remains `false`, which is the default value. |   Optional   |
|         `samples`         | Number of complex samples per channel to be read. If set to $$ 0 $$, the whole file but the last two milliseconds are processed. It defaults to $$ 0 $$. |   Optional   |
|      `bytes_to_skip`      | Number of bytes to skip from the beginning of the file. If omitted, the block attempts to find the end of the embedded header and startup synchronization pattern automatically. If auto-detection fails, the legacy fallback is `65536` bytes. |   Optional   |
|         `repeat`          | [`true`, `false`]: If set to `true`, processing of samples restarts the file when the end is reached. It defaults to `false`. |   Optional   |
|          `dump`           | [`true`, `false`]: If set to `true`, dumps the converted <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr> samples. It defaults to `false`. |   Optional   |
|      `dump_filename`      | Base path of the dump file. The source appends `_ch0`, `_ch1`, and so on for each decoded file channel. It defaults to `./my_capture_dump.dat`. |   Optional   |
| `enable_throttle_control` | [`true`, `false`]: If set to `true`, it places a throttle controlling the data flow. It is generally not required, and it defaults to `false`. |   Optional   |
|          -------          |

  _Signal Source implementation:_ **`GSS6450_File_Signal_Source`**
  {: style="text-align: center;"}

Example using auto-detection of the embedded header, bit depth, file channel
count, and IQ data offset:

```ini
;######### SIGNAL_SOURCE CONFIG ############
SignalSource.implementation=GSS6450_File_Signal_Source
SignalSource.filename=/home/gnss/data/my_capture.A.gns
SignalSource.sampling_frequency=30690000
```

Example selecting only the second recorded channel from a two-channel `.A.gns`
file:
...
SignalSource.sel_ch=2
```

Example exposing both recorded channels from a dual-band `.A.gns` file. This
creates two RF channels in GNSS-SDR, each connected to its own Signal Conditioner
chain:

```ini
;######### SIGNAL_SOURCE CONFIG ############
SignalSource.implementation=GSS6450_File_Signal_Source
SignalSource.filename=/home/gnss/data/dual_band_capture.A.gns
SignalSource.sampling_frequency=30690000
SignalSource.total_channels=2
SignalSource.RF_channels=2
```

Example exposing three recorded channels from a GSS6425 compatibility-mode file.
If the file contains the embedded `<Header> GSS6425` header, `adc_bits=2` and
`gss6425_compatible=true` can be auto-detected; they are shown explicitly here
for clarity:

```ini
;######### SIGNAL_SOURCE CONFIG ############
SignalSource.implementation=GSS6450_File_Signal_Source
SignalSource.filename=/home/gnss/data/three_band_compatibility.gns
SignalSource.sampling_frequency=30690000
SignalSource.total_channels=3
SignalSource.RF_channels=3
SignalSource.adc_bits=2
SignalSource.gss6425_compatible=true
```

For a four-band GSS6450 recording, configure two signal sources: one for the
`.A.gns` file and one for the `.B.gns` file. Each source can expose one or two RF
channels, depending on the corresponding file contents.

```ini
;######### SIGNAL_SOURCE CONFIG ############
SignalSource0.implementation=GSS6450_File_Signal_Source
SignalSource0.filename=/home/gnss/data/four_band_capture.A.gns
SignalSource0.sampling_frequency=30690000
SignalSource0.total_channels=2
SignalSource0.RF_channels=2

SignalSource1.implementation=GSS6450_File_Signal_Source
SignalSource1.filename=/home/gnss/data/four_band_capture.B.gns
SignalSource1.sampling_frequency=30690000
SignalSource1.total_channels=2
SignalSource1.RF_channels=2
```
...
LabSat devices record and replay real-world raw sample data, allowing
...
extract selected logical sample streams on demand.

A metadata file `.sdrx` provides a structured representation of one or more sample
streams and the binary file or files where those streams are stored. The precise
location and manner in which the data is stored are defined within the metadata
file, including quantization, alignment, shifts, packing, chunk structure, and
stream rate factors. The binary file URL declared in the metadata is resolved
relative to the directory containing the metadata file.

Each stream within the metadata file is identified by a string ID. The user
supplies a comma-separated list of those stream IDs through the `streams`
parameter. The selected streams are decoded and exposed as distinct output
streams, which enables multi-band receivers when the metadata file describes
more than one band. Repeated stream IDs in the list are ignored.

GNSS-SDR determines the output item size from the metadata. For integer
encodings, intermediate frequency (`IF`) streams are delivered as scalar sample
streams. Integer complex stream formats such as `IQ` or `QI` are delivered as
interleaved scalar I/Q items, so the Signal Conditioner must use a compatible
[Data Type Adapter]({{ "/docs/sp-blocks/data-type-adapter/" | relative_url }}).
For example, a decoded 8-bit real IF stream can be connected to `Byte_To_Short`,
whereas an 8-bit interleaved I/Q stream can be connected to `Ibyte_To_Complex`.

For floating-point (`FP`) encodings, this implementation supports 32-bit
IEEE-754 samples. FP32 `IF` streams are delivered as `float` items, and FP32
complex streams are delivered directly as `gr_complex` items, one item per I/Q
sample.

The implementation supports integer stream encodings described by the standard,
including `SIGN`, `OB`, `SM`, `MS`, `TC`, `OG`, `OBA`, `SMA`, `MSA`, `TCA`, and
`OGA`. FP64 stream encodings are not currently supported.
...
|   `metadata_filename`    | Path to the XML metadata file, usually with `.sdrx` extension. Data file URLs in the metadata are resolved relative to this file.              |  Mandatory   |
|        `streams`         | Comma-separated list of logical stream IDs to decode, for example `L1` or `L1,L5`. Whitespace is ignored.                                      |  Mandatory   |
|     `RF_channels`        | Number of decoded streams exposed as RF channels. Set it to the number of stream IDs in `streams` for multi-band configurations. It defaults to `1`. |   Optional   |
|          -------         |
...
Example of a dual-band receiver using one metadata file that describes two
logical streams. The first stream listed in `SignalSource.streams` is exposed as
RF channel `0`, and the second one as RF channel `1`. This is still a single
Signal Source; `GNSS-SDR.num_sources` can be left at its default value of `1`:

```ini
;######### SIGNAL_SOURCE CONFIG ############
SignalSource.implementation=ION_GSMS_Signal_Source
SignalSource.metadata_filename=data/dual_band_capture.sdrx
SignalSource.streams=L1,L5
SignalSource.RF_channels=2

;######### SIGNAL_CONDITIONER CONFIG ############
; This example assumes that both metadata streams are FP32 complex streams,
; which the source delivers as gr_complex items.
SignalConditioner0.implementation=Pass_Through
SignalConditioner0.item_type=gr_complex

SignalConditioner1.implementation=Pass_Through
SignalConditioner1.item_type=gr_complex
...
For integer complex streams, replace each `Pass_Through` Signal Conditioner with
a Signal Conditioner chain whose Data Type Adapter matches the decoded metadata
output, for example `Ibyte_To_Complex` or `Ishort_To_Complex`.
...
### Implementation: `EVK1029_Signal_Source`

**EVK1029** is an Evaluation Kit for the space-grade ASICs
designed by [SAPHYRION SAGL](https://saphyrion.ch/): the SY1009 RF front-end and
the SY1019 ADC/DSP. The kit consists of a USB 3.0 device connected to a
dual-band (E1/E5a) or triple-band (E1/E5a/E6) GNSS front-end board, plus a Linux
host application that controls and monitors the device and saves the captured
real-time sample stream to a binary file. This Signal Source reads those raw
capture files directly.

The capture format is a continuous, header-less stream of real-valued IF
samples, quantized with 4 bits and encoded as offset-binary-adjusted (`OBA`),
packed two per byte (low nibble first) and 16 samples per little-endian 64-bit
word. Although this layout can be described with an `.sdrx` metadata file and
read through the
[`ION_GSMS_Signal_Source`](#implementation-ion_gsms_signal_source), the
dedicated reader is simpler to configure and cheaper to run at the raw ADC rate
of the kit (up to about $$ 180 $$ Msps). Each 4-bit sample is unpacked into one
signed byte taking the odd values $$ \{-15, -13, \dots, -1, 1, \dots, 15\} $$,
so the source delivers a stream of items of type <abbr id="data-type"
title="Signed integer, 8-bit two's complement number ranging from -128 to 127.
C++ type name: int8_t">`byte`</abbr>. The Signal Conditioner must therefore
start with a Data Type Adapter accepting `byte` items (`Pass_Through` or
`Byte_To_Short`), followed by a `Freq_Xlating_Fir_Filter` that shifts the
...
Example of a single-band Galileo E1 / GPS L1 receiver reading an EVK1029
capture. The Input Filter shifts the E1/L1 band from its intermediate frequency
to baseband and decimates the raw stream to the working rate:

```ini
[GNSS-SDR]
GNSS-SDR.internal_fs_sps=22250000
...
InputFilter.sampling_frequency=178000000
InputFilter.IF=0      ; <- PUT THE E1/L1 IF OF YOUR CAPTURE HERE
...
InputFilter0.sampling_frequency=178000000
InputFilter0.IF=0     ; <- PUT THE E1/L1 IF OF YOUR CAPTURE HERE
InputFilter0.decimation_factor=8
InputFilter0.filter_type=lowpass
Resampler0.implementation=Pass_Through
Resampler0.item_type=gr_complex

;######### SIGNAL_CONDITIONER 1 (E5a / L5) ############
SignalConditioner1.implementation=Signal_Conditioner
DataTypeAdapter1.implementation=Pass_Through
DataTypeAdapter1.item_type=byte
InputFilter1.implementation=Freq_Xlating_Fir_Filter
InputFilter1.input_item_type=byte
InputFilter1.output_item_type=gr_complex
InputFilter1.taps_item_type=float
InputFilter1.sampling_frequency=178000000
InputFilter1.IF=0     ; <- PUT THE E5a/L5 IF OF YOUR CAPTURE HERE
