...
In some applications, it can be useful to get the Time of Week and Week Number at the Tracking blocks, when available.

Activating this configuration parameter makes this information available both in the dump files generated by the Tracking blocks and in the `Gnss_Synchro` objects they emit.

|----------
| **Parameter**  | **Description**                                   | **Required** |
| :------------: | :------------------------------------------------ | :----------: |
| `tow_to_trk` | [`true`, `false`]: When set to `true`, Telemetry blocks send asynchronous messages back to the Tracking blocks containing the information required to compute the TOW and week number at the tracking stage. It defaults to `false`. |   Optional   |
| -------------- |
