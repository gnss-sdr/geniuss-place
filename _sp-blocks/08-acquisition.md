...
* In multi-frequency configurations, the Doppler measured by a channel tracking
  ...
  This is controlled by `GNSS-SDR.assist_dual_frequency_acq`. A single
  signal can opt out of this narrowing with
  `Acquisition_XX.dual_freq_assisted_doppler_narrowing=false` (it defaults to
  `true`): its searches are then always performed over the full grid centered
  at $$ 0 $$ Hz, while the other secondary bands remain assisted. Only the
  projected Doppler is discarded: with
  `GNSS-SDR.assist_dual_frequency_acq=true`, the secondary band still waits
  until the satellite is tracked in the primary band, and searches for that
  same satellite. This is mainly intended for characterizing the unassisted
  acquisition performance of a given band without disabling the assistance for
  the rest of the receiver. The parameter is read from the role the block was
  configured with, so it can also be set on a per-channel role (_e.g._,
  `Acquisition_L50.dual_freq_assisted_doppler_narrowing=false`) when those are
  in use (see [Channels]({{ "/docs/sp-blocks/channels/" | relative_url }})).
...
from within the grid. Adding
`Acquisition_L5.dual_freq_assisted_doppler_narrowing=false` would make every
L5 search use this full grid, even when the satellite is already tracked in
L1 C/A.
...
|:-:|:--|:-:|
|--------------
| `implementation` | `BEIDOU_B1I_PCPS_Acquisition` | Mandatory |
| `item_type` | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 16-bit integer. C++ name: lv_16sc_t (custom definition of std::complex<int16_t>)">`cshort`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 8-bit integer. C++ name: lv_8sc_t (custom definition of std::complex<int8_t>)">`cbyte`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>. | Optional |
| `doppler_max`  | Maximum Doppler value in the search grid, in Hz. It defaults to 5000 Hz. | Optional |
| `doppler_step` | Frequency step in the search grid, in Hz. It defaults to 500 Hz. | Optional |
| `threshold`    |  Decision threshold $$ \gamma $$ from which a signal will be considered present. It defaults to $$ 0.0 $$ (_i.e._, all signals are declared present). | Optional |
...
|--------------
  ...
  _Acquisition implementation:_ **`BEIDOU_B1I_PCPS_Acquisition`**.
  {: style="text-align: center;"}
...
## BeiDou B1C signal acquisition

The BeiDou B1C signal, transmitted by BeiDou-3 MEO and IGSO satellites and
centered at $$ 1575.42 $$ MHz, is composed of a data component (carrying the
B-CNAV1 navigation message and modulated with BOC(1,1)) and a pilot component
(modulated with QMBOC(6,1,4/33)), with a data:pilot power ratio of 1:3. Both
components use ranging codes of $$ 10230 $$ chips transmitted at $$ 1.023 $$
Mchip/s, thus with a period of $$ 10 $$ ms, and the pilot component is further
modulated by a secondary code of $$ 1800 $$ chips. GNSS-SDR identifies the
...
`acquire_pilot` and `qmboc` parameters allow acquiring the data component
instead, and/or using a simpler sinBOC(1,1) local replica.
...
|:-:|:--|:-:|
|--------------
| `implementation` | `BEIDOU_B1C_PCPS_Ambiguous_Acquisition` | Mandatory |
| `item_type` | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 16-bit integer. C++ name: lv_16sc_t (custom definition of std::complex<int16_t>)">`cshort`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 8-bit integer. C++ name: lv_8sc_t (custom definition of std::complex<int8_t>)">`cbyte`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>. | Optional |
| `acquire_pilot` | [`true`, `false`]: If set to `true`, the receiver is set to acquire the B1C pilot component. If set to `false`, the receiver is set to acquire the B1C data component. It defaults to `true`. | Optional |
| `qmboc` | [`true`, `false`]: If set to `true`, the algorithm uses a QMBOC(6,1,4/33) local replica; if set to `false`, a simpler sinBOC(1,1) local replica is used. It defaults to `true`. | Optional |
| `doppler_max`  | Maximum Doppler value in the search grid, in Hz. It defaults to 5000 Hz. | Optional |
| `doppler_step` | Frequency step in the search grid, in Hz. It defaults to 500 Hz. | Optional |
| `threshold`    |  Decision threshold $$ \gamma $$ from which a signal will be considered present. It defaults to $$ 0.0 $$ (_i.e._, all signals are declared present). | Optional |
| `pfa` |  If defined, it supersedes the `threshold` value and computes a new threshold $$ \gamma_{pfa} $$ based on the Probability of False Alarm. It defaults to $$ 0.0 $$ (_i.e._, not set). | Optional |
...
| `second_doppler_step` | If `make_two_steps` is set to `true`, this parameter sets the Doppler step applied in the acquisition refinement stage, in Hz. It defaults to 125 Hz. | Optional |
| `dump` |  [`true`, `false`]: If set to `true`, it enables the Acquisition internal binary data file logging. It defaults to `false`. | Optional |
| `dump_filename` |  If `dump` is set to `true`, name of the file in which internal data will be stored. This parameter accepts either a relative or an absolute path; if there are non-existing specified folders, they will be created. It defaults to `./acquisition`, so files with name `./acquisition_C_1D_ch_N_K_sat_P.mat` (where `N` is the channel number defined by `dump_channel`, `K` is the dump number, and `P` is the targeted satellite's PRN number) will be generated. | Optional |
| `dump_channel` | If `dump` is set to `true`, channel number from which internal data will be stored. It defaults to 0. | Optional |
|--------------

  _Acquisition implementation:_ **`BEIDOU_B1C_PCPS_Ambiguous_Acquisition`**.
  {: style="text-align: center;"}

Example:

```ini
;######### ACQUISITION CONFIG FOR BeiDou B1C CHANNELS ############
...
|:-:|:--|:-:|
|--------------
| `implementation` | `QZSS_L1_PCPS_Acquisition` | Mandatory |
| `item_type` | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 16-bit integer. C++ name: lv_16sc_t (custom definition of std::complex<int16_t>)">`cshort`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 8-bit integer. C++ name: lv_8sc_t (custom definition of std::complex<int8_t>)">`cbyte`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>. | Optional |
| `doppler_max`  | Maximum Doppler value in the search grid, in Hz. It defaults to 5000 Hz. | Optional |
| `doppler_step` | Frequency step in the search grid, in Hz. It defaults to 500 Hz. | Optional |
| `threshold`    |  Decision threshold $$ \gamma $$ from which a signal will be considered present. It defaults to $$ 0.0 $$ (_i.e._, all signals are declared present). | Optional |
...
|--------------
  ...
  _Acquisition implementation:_ **`QZSS_L1_PCPS_Acquisition`**.
  {: style="text-align: center;"}
...
|:-:|:--|:-:|
|--------------
| `implementation` | `BEIDOU_B3I_PCPS_Acquisition` | Mandatory |
| `item_type` | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 16-bit integer. C++ name: lv_16sc_t (custom definition of std::complex<int16_t>)">`cshort`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 8-bit integer. C++ name: lv_8sc_t (custom definition of std::complex<int8_t>)">`cbyte`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>. | Optional |
| `doppler_max`  | Maximum Doppler value in the search grid, in Hz. It defaults to 5000 Hz. | Optional |
| `doppler_step` | Frequency step in the search grid, in Hz. It defaults to 500 Hz. | Optional |
| `threshold`    |  Decision threshold $$ \gamma $$ from which a signal will be considered present. It defaults to $$ 0.0 $$ (_i.e._, all signals are declared present), | Optional |
...
|--------------
  ...
  _Acquisition implementation:_ **`BEIDOU_B3I_PCPS_Acquisition`**.
  {: style="text-align: center;"}
...
|:-:|:--|:-:|
|--------------
| `implementation` | `QZSS_L5i_PCPS_Acquisition` | Mandatory |
| `item_type` | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 16-bit integer. C++ name: lv_16sc_t (custom definition of std::complex<int16_t>)">`cshort`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 8-bit integer. C++ name: lv_8sc_t (custom definition of std::complex<int8_t>)">`cbyte`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>. | Optional |
| `doppler_max`  | Maximum Doppler value in the search grid, in Hz. It defaults to 5000 Hz. | Optional |
| `doppler_step` | Frequency step in the search grid, in Hz. It defaults to 500 Hz. | Optional |
| `threshold`    |  Decision threshold $$ \gamma $$ from which a signal will be considered present. It defaults to $$ 0.0 $$ (_i.e._, all signals are declared present), | Optional |
...
|--------------
  ...
  _Acquisition implementation:_ **`QZSS_L5i_PCPS_Acquisition`**.
  {: style="text-align: center;"}
...
## BeiDou B2a signal acquisition

The BeiDou B2a signal, transmitted by BeiDou-3 MEO and IGSO satellites and
centered at $$ 1176.45 $$ MHz, is composed of a data component (carrying the
B-CNAV2 navigation message) and a pilot component, in phase quadrature and
with equal power, both modulated with BPSK(10). Both components use ranging
codes of $$ 10230 $$ chips transmitted at $$ 10.23 $$ Mchip/s, thus with a
period of $$ 1 $$ ms. The data component is further modulated by a
$$ 5 $$-chip secondary code, and the pilot component by a $$ 100 $$-chip
secondary code. GNSS-SDR identifies the BeiDou B2a signal with the identifier
`5D`.
...
default coherent integration time of $$ 1 $$ ms avoids straddling secondary
code chip transitions; sensitivity can be increased by accumulating
non-coherent dwells with `max_dwells`.
...
|:-:|:--|:-:|
|--------------
| `implementation` | `BEIDOU_B2A_PCPS_Acquisition` | Mandatory |
| `item_type` | [<abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 16-bit integer. C++ name: lv_16sc_t (custom definition of std::complex<int16_t>)">`cshort`</abbr>, <abbr id="data-type" title="Complex samples with real and imaginary parts of type signed 8-bit integer. C++ name: lv_8sc_t (custom definition of std::complex<int8_t>)">`cbyte`</abbr>]: Set the sample data type expected at the block input. It defaults to <abbr id="data-type" title="Complex samples with real and imaginary parts of type 32-bit floating point. C++ name: std::complex<float>">`gr_complex`</abbr>. | Optional |
| `doppler_max`  | Maximum Doppler value in the search grid, in Hz. It defaults to 5000 Hz. | Optional |
| `doppler_step` | Frequency step in the search grid, in Hz. It defaults to 500 Hz. | Optional |
| `threshold`    |  Decision threshold $$ \gamma $$ from which a signal will be considered present. It defaults to $$ 0.0 $$ (_i.e._, all signals are declared present). | Optional |
...
| `second_doppler_step` | If `make_two_steps` is set to `true`, this parameter sets the Doppler step applied in the acquisition refinement stage, in Hz. It defaults to 125 Hz. | Optional |
| `dump` |  [`true`, `false`]: If set to `true`, it enables the Acquisition internal binary data file logging. It defaults to `false`. | Optional |
| `dump_filename` |  If `dump` is set to `true`, name of the file in which internal data will be stored. This parameter accepts either a relative or an absolute path; if there are non-existing specified folders, they will be created. It defaults to `./acquisition`, so files with name `./acquisition_C_5D_ch_N_K_sat_P.mat` (where `N` is the channel number defined by `dump_channel`, `K` is the dump number, and `P` is the targeted satellite's PRN number) will be generated. | Optional |
| `dump_channel` | If `dump` is set to `true`, channel number from which internal data will be stored. It defaults to 0. | Optional |
|--------------

  _Acquisition implementation:_ **`BEIDOU_B2A_PCPS_Acquisition`**.
  {: style="text-align: center;"}

Example:

```ini
;######### ACQUISITION CONFIG FOR BeiDou B2a CHANNELS ############
Acquisition_5D.implementation=BEIDOU_B2A_PCPS_Acquisition
Acquisition_5D.item_type=gr_complex
Acquisition_5D.coherent_integration_time_ms=1
Acquisition_5D.pfa=0.01
Acquisition_5D.doppler_max=5000
Acquisition_5D.doppler_step=250
Acquisition_5D.max_dwells=8
```
