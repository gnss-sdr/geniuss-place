...
* ![GNU/kFreeBSD Logo](https://www.file-extensions.org/imgs/app-icon/128/3219/freebsd-icon.png){:height="100px" width="100x"}{: .align-right} **GNU/kFreeBSD**: GNSS-SDR can be built in GNU distributions which make use of the kernel of [FreeBSD](https://www.freebsd.org){:target="_blank"} instead of the Linux kernel. This is the case of [Debian GNU/kFreeBSD](https://wiki.debian.org/Debian_GNU/kFreeBSD){:target="_blank"}, a port that consists of the GNU userland using the GNU C library on top of FreeBSD's kernel, coupled with the regular Debian package set.
